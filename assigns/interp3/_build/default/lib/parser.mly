...
ty:
  ...
  | t1=ty; ARROW; t2=ty          { TFun (t1, t2) }   /* change TFun if your AST uses another name */
...
expr:
  ...
  (* IF / MATCH will go here later if required *)
  | e = expr2 { e }
...
%inline bop:
  ...
  | LTE  { Lte }
  | GT   { Gt }
  | GTE  { Lte }  (* if your AST has Gte instead, use Gte *)
  | EQ   { Eq }
  | NEQ  { Neq }
  | AND  { And }
  | OR   { Or }
